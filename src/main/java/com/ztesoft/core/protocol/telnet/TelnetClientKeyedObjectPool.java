...
/**
 * <Description>TelnetClient连接池持有者 <br>
 * 目前本连接池以（ServerConfig）为KEY，（ServerConfig）代表不同的主机（以host,user,port为维度判定），每个KEY对应一个连接池组，通过Config控制池的大小、超时等配置<br>
 * 注意：对于不同的key，将使用一样的配置，并且第一次实例化TelnetClientKeyedObjectPool时就生效，后面的key都沿用这个配置
 * 
...
public class TelnetClientKeyedObjectPool {
    private KeyedObjectPool<ServerConfig, TelnetClient> pool;

    private org.apache.commons.pool.impl.GenericKeyedObjectPool.Config config;

    private static class SingletonHolder {
        public static TelnetClientKeyedObjectPool INSTANCE;
    }
     ...
     * @return
     */
    public synchronized static TelnetClientKeyedObjectPool getInstance() {
        if (null != SingletonHolder.INSTANCE)
            return SingletonHolder.INSTANCE;
        SingletonHolder.INSTANCE = new TelnetClientKeyedObjectPool();
        return SingletonHolder.INSTANCE;
    }
    ...
    public synchronized static TelnetClientKeyedObjectPool getInstance(
        ...
        if (null != SingletonHolder.INSTANCE)
            return SingletonHolder.INSTANCE;
        SingletonHolder.INSTANCE = new TelnetClientKeyedObjectPool(config);
        return SingletonHolder.INSTANCE;
    }
     ...
     * 使用默认配置实例化连接池
     */
    private TelnetClientKeyedObjectPool() {
        ...
        this.config = config;
        pool = new GenericKeyedObjectPool<ServerConfig, TelnetClient>(
                new TelnetClientFactory(), config);
    }
     ...
     * @return
     */
    public synchronized KeyedObjectPool<ServerConfig, TelnetClient> getPool() {
        return pool;
    }
